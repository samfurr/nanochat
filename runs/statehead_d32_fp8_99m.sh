...
# StateHead-only d32 FP8 attempt at the current 99-minute nanochat
# time-to-GPT-2 budget. Run on one already-provisioned 8x H100 SXM node.
# This script does not create or destroy infrastructure.

set -euo pipefail

readonly MODEL_CODE_COMMIT="f68feae68a6be965c8f76c40026cb134aba2c216"
...
echo "StateHead d32 FP8 99-minute attempt"
...
if [[ "$DRY_RUN" == "1" ]]; then
    ...
    print_command env NANOCHAT_DTYPE=bfloat16 python -m torch.distributed.run \
        --standalone "--nproc_per_node=$WORLD_SIZE" --module dev.statehead_cuda_preflight \
        --arch=statehead "--device-batch-size=$DEVICE_BATCH_SIZE" --steps=4 \
        --warmup-steps=1 "--layers=$DEPTH" "--model-width=$MODEL_WIDTH" \
        "--heads=$STATE_HEADS" "--sequence-length=$SEQUENCE_LENGTH" \
        "--scan-chunk-size=$SCAN_CHUNK_SIZE" --scan-backend=cuda --fp8 \
        --verify-gradients --output="$RESULTS_DIR/statehead-d32-fp8-ddp-gate.json"
    build_train_command statehead-d32-fp8-calibration "$GATE_STEPS" \
        "$GATE_EVAL_EVERY" "$GATE_EVAL_TOKENS" -1
...
nvidia-smi --query-gpu=name,memory.total,power.limit,driver_version --format=csv | tee "$RESULTS_DIR/nvidia-smi.csv"
...
if losses[-1] >= losses[0]:
    raise SystemExit(f"training loss did not decrease: first={losses[0]} last={losses[-1]}")
...
[[ "$TRAINING_STEPS" =~ ^[0-9]+$ ]] || fail "invalid calibrated training steps: $TRAINING_STEPS"
...
NANOCHAT_DTYPE=bfloat16 "${TRAIN_COMMAND[@]}" 2>&1 | tee "$RESULTS_DIR/${FULL_TAG}-train.log"
echo "$((SECONDS - FULL_START_SECONDS))" > "$RESULTS_DIR/${FULL_TAG}-train-wall-seconds.txt"

FINAL_MODEL="$FULL_CHECKPOINT_DIR/model_${FINAL_STEP_PADDED}.pt"
FINAL_META="$FULL_CHECKPOINT_DIR/meta_${FINAL_STEP_PADDED}.json"
[[ -f "$FINAL_MODEL" ]] || fail "final model checkpoint missing: $FINAL_MODEL"
[[ -f "$FINAL_META" ]] || fail "final metadata missing: $FINAL_META"
...
NANOCHAT_DTYPE=bfloat16 "${EVAL_COMMAND[@]}" 2>&1 | tee "$RESULTS_DIR/${FULL_TAG}-eval.log"

CORE_CSV="$NANOCHAT_BASE_DIR/base_eval/base_model_${FINAL_STEP_PADDED}.csv"
[[ -f "$CORE_CSV" ]] || fail "final CORE CSV missing: $CORE_CSV"
cp "$CORE_CSV" "$RESULTS_DIR/${FULL_TAG}-core.csv"
cp "$FINAL_META" "$RESULTS_DIR/${FULL_TAG}-meta_${FINAL_STEP_PADDED}.json"
cp "$FINAL_MODEL" "$RESULTS_DIR/${FULL_TAG}-model_${FINAL_STEP_PADDED}.pt"
shopt -s nullglob
optimizer_shards=("$FULL_CHECKPOINT_DIR"/optim_"${FINAL_STEP_PADDED}"_rank*.pt)
shopt -u nullglob
[[ "${#optimizer_shards[@]}" == "$WORLD_SIZE" ]] || fail "expected $WORLD_SIZE final optimizer shards, found ${#optimizer_shards[@]}"
sha256sum \
    "$RESULTS_DIR/${FULL_TAG}-core.csv" \
    "$RESULTS_DIR/${FULL_TAG}-meta_${FINAL_STEP_PADDED}.json" \
    "$RESULTS_DIR/${FULL_TAG}-model_${FINAL_STEP_PADDED}.pt" \
    "${optimizer_shards[@]}" \
    > "$RESULTS_DIR/${FULL_TAG}.sha256"
